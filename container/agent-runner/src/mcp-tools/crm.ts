 ...
 */
import { checkContactLock, setResearchPhase } from './research-budget.js';
...
export const crmGetNextProspect: McpToolDefinition = {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        max_years_licensed: {
          type: 'integer',
          description:
            'Only return a prospect licensed this many years or fewer. Check the system prompt / your instructions for the current threshold rather than guessing one.',
        },
        min_years_licensed: {
          type: 'integer',
          description:
            'Only return a prospect licensed at least this many years. Check the system prompt / your instructions for the current threshold rather than guessing one.',
        },
      },
    },
  },
  async handler(args) {
    ...
    if (args.min_years_licensed !== undefined) params.set('min_years_licensed', String(args.min_years_licensed));
...
export const crmEnrichContact: McpToolDefinition = {
  ...
  async handler(args) {
    ...
    const lockMsg = checkContactLock(contact_id);
    if (lockMsg) {
      log(`crm_enrich_contact -> id ${contact_id} rejected (contact lock)`);
      return ok(lockMsg);
    }
