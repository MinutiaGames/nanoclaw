...
## v7 — current (2026-08-16, this is what's live in run-crm-batch.sh)

Flipped the phase-1 pool filter from "5 years licensed or fewer" to "5 years
licensed or more" — `crm_get_next_prospect` now called with
`min_years_licensed: 5` instead of `max_years_licensed: 5`, prioritizing
established licensees over newer ones. Only the filter direction/wording
changed; everything else (search-cap discipline, LinkedIn-capture, name-
mismatch verification) is unchanged from v6. Supporting `min_years_licensed`
filter (`years_licensed >= N`) added this session to `crm_get_next_prospect`'s
MCP schema (`container/agent-runner/src/mcp-tools/crm.ts`), the CRM's
`getNextProspect` (`/mnt/c/claude/leadgen-crm/lib/contacts.ts`), and its API
route (`app/api/contacts/next/route.ts`) — `max_years_licensed` was left in
place (not removed) so either direction stays available for future
threshold experiments. Not yet validated against a live batch as of this
writing.

```
Use the crm_get_next_prospect tool (contact_type: referral_partner, min_years_licensed: 5) to pick one CPA from the CRM who hasn't been researched yet and has been licensed at least 5 years — established licensees are the current priority, don't omit this filter. It will give you their real name, firm, and address already verified — do not question or re-derive that part. Then use web_search and web_fetch to research that specific person or firm: look for their firm's website, a public email or phone number, how long they've been in practice, and anything else useful (client reviews, specialties, social media). Lead with your MOST specific search first — the full name in quotes plus "CPA" plus their city/state or license number — rather than a bare name search; a common first name or everyday word (e.g. "Adam", "Jordan", "Cassandra", "Killian") reliably pulls in unrelated pages (Wikipedia name/word entries, mythology, geography, brands) that have nothing to do with this person. If you notice that happening — results about the word/name itself rather than a CPA or a Florida license — that's your cue to stop, not to keep rephrasing the same search. Cap yourself at 3 web_search calls for this contact: if none of them turn up a clearly-matching, individual result, stop searching entirely and call crm_enrich_contact right away with just the CRM-provided fields, status 'researched', and a note like "common name/no individual web footprint found" — do not keep retrying variations of the same query. A contact saved with minimal info beats one left unresearched because search never found a good angle. Don't bother fetching people-search/background-check sites (Whitepages, Spokeo, Radaris, BeenVerified, TruePeopleSearch, Intelius, MyLife, and similar) even if they show up in search results — those are paywalled and show masked placeholder data to non-subscribers, not real facts. Same goes for LinkedIn personal-profile URLs (linkedin.com/in/...) — web_fetch refuses these outright since they always hit a login wall; use the snippet text web_search already gave you for that result instead of trying to fetch the page. Even though you can't fetch it, when a linkedin.com/in/... result for this specific person appears in ANY of your search results, ALWAYS capture the exact URL and save it — set signals.social_handles to {"linkedin": "<url>"} (this flows into its own linkedin_url field for a human to review manually later). Don't skip past a LinkedIn result just because you can't read the full page — the URL alone is worth saving even when nothing else on it is accessible to you. LinkedIn company pages (linkedin.com/company/...) are fine to fetch and often have real info. When you're done researching, call crm_enrich_contact with their contact_id and ONLY the fields you actually found — leave a field out entirely if you couldn't find it, do not guess or invent a plausible-looking value. Put any freeform findings (years in business, review rating, etc.) in the signals object. Set status to 'researched' once you've made a genuine attempt, even if you found little. Then send me a short summary of what you found and saved.
```

## v6 (2026-08-10 – 2026-08-16, superseded by v7 above)

Added one instruction after a manual review session: going through 13
contacts caught by the extra-touch bug (see `HANDOFF.md` 2026-08-09/10),
the user found real LinkedIn profiles for several by simply searching
`"[First Name] [Last Name] CPA"` — the same query the prompt already tells
the model to lead with — that the model itself never surfaced. The model
was already told not to *fetch* `linkedin.com/in/...` URLs (they always
hit a login wall), but that instruction apparently reads as "ignore
LinkedIn results entirely" rather than "capture the URL, just don't fetch
it" — so a real, findable lead source was going unrecorded even when it
showed up in the model's own search results. Fix: an explicit sentence
telling the model to always save the exact URL (via
`signals.social_handles = {"linkedin": "<url>"}`, which the CRM already
promotes into its own `linkedin_url` column) whenever a personal-profile
result for this person appears — the URL alone is useful for a human to
follow up on manually, even with zero extractable page content. Not a new
search step, just making sure an already-surfaced result stops getting
silently discarded. Not yet validated against a live batch as of this
writing.
...
## v5 (2026-08-01 – 2026-08-10, superseded by v6 above)
...
### v7 — current (2026-08-16, this is what's live in run-crm-batch.sh's PROMPT_PHASE2)

Rina (the firm's actual bookkeeper/end-user of these referral partners)
reviewed `c:/claude/phase2-referral-partner-criteria-for-rina.md` — the
plain-language writeup of what phase 2 looks for — and added her own
criteria plus an answer to the doc's open question. Changes, all additive
(no existing hard disqualifiers changed):

1. **New strong positive signal, item (4)**: explicit referral-partnership
   language ("we partner with bookkeepers", "we refer clients to
   bookkeeping firms," etc.) is now checked for explicitly and treated as
   on par with trade-client evidence — previously this was only implicitly
   covered as "whatever else you found."
2. **Three new soft/ranking signals, none disqualifying**: (a) remote/
   virtual vs. in-person-emphasis (Rina: "we're remote, don't want someone
   emphasizing in-person"); (b) accounting software mentioned, QuickBooks
   Online preferred, Desktop/other noted but not penalized as a hard
   disqualifier (Rina: "not ideal, though also not a hard disqualifier,
   simply a low rank"); (c) communication-quality signals (Rina: "a lot of
   CPAs have very minimal communication, and we don't want that"). All
   three are captured as new freeform `signals` keys
   (`remote_friendly`, `accounting_software`, `communication_quality`) and
   are meant to shape how strong a `potential` verdict reads in `note`, not
   change the binary potential/not_viable field itself — there's no numeric
   ranking field in the schema, so the model is told to spell out stacked
   positive signals in prose for a human to prioritize on.
3. **Open question from the doc resolved, no prompt change needed**: the
   doc asked whether "couldn't find enough to justify pursuing" should be
   a distinct outcome from "actually disqualified," rather than both
   collapsing into `not_viable`. Rina's answer: *"Yes, if substantive
   information is not found, that disqualifies them."* This confirms the
   prompt's existing behavior (`not_viable` "if a hard disqualifier was
   found or there's simply no positive signal to justify pursuing them")
   is already correct as designed — nothing to change here.

Not yet validated against a live batch as of this writing.

```
Use the crm_get_phase2_candidates tool (contact_type: referral_partner) to pull 5 already-researched CPAs from the CRM. Using ONLY the information already returned for each of the 5 — do not search the web yet — pick the single most promising one as a referral-partner candidate: favor whoever looks strongest on audience fit and reach (years_in_business, client_count_est, review_rating/review_count, accepting_new_clients, any notes hinting at trade-client work). If several look similar, picking any reasonable one is fine, this is a coarse triage not a precise ranking. Then research ONLY that one contact — leave the other 4 completely untouched, they go back in the pool for a future run. Look specifically for: (1) whether the firm offers bookkeeping services itself — check the site's nav/services list for anything like "Bookkeeping Services"; if you see it, open that SPECIFIC page and read it, don't just infer from the label or move on. If the page describes them doing the bookkeeping work themselves (their own staff/process, no mention of outsourcing or referring it elsewhere), that's a hard disqualifier — they're a competitor, not a referral source. Only treat it as NOT disqualifying if the page explicitly says they outsource or refer bookkeeping to another firm/partner. If that specific page 404s or won't load but bookkeeping is still listed as one of their services elsewhere on the site (nav, homepage, service list), default to treating it as in-house and disqualify rather than leaving it unresolved — a firm advertising a service on its own site is offering it themselves unless stated otherwise; (2) whether the firm explicitly states it doesn't make referrals to other professionals — rare, but also a hard disqualifier if found; (3) whether the firm serves trade/contractor clients (HVAC, plumbing, electrical, construction) — check their site's services/"who we serve" page, case studies, testimonials; this is the strongest positive signal, actively look for it; (4) whether the firm explicitly states it partners with or refers to bookkeepers/bookkeeping firms, or that it makes referrals to other professionals generally — this is also a strong positive signal, on par with trade-client evidence. Beyond those, look for three more signals that shape how strong a 'potential' verdict is without being disqualifying on their own: whether the firm operates fully remote/virtual versus emphasizing in-person/local-only service (we're a remote firm and a CPA who leans heavily in-person is a weaker fit, though not a hard no); what accounting software they mention using (we're on QuickBooks Online — QuickBooks Desktop or another platform isn't ideal but isn't disqualifying, just note whatever you find); and any evidence of communication quality (many CPA sites read as minimal/unresponsive — look for signs of active, responsive communication: a real contact form or live chat, testimonials that specifically praise responsiveness, an active blog/social presence; treat a clear positive signal here as strengthening a 'potential' verdict, and a clear negative sign as weakening but not disqualifying it). Note: a firm NOT currently accepting new clients is NOT by itself a hard disqualifier — they can still be a great referral source, possibly even more motivated to refer people elsewhere since they can't take them on directly; only the two items in (1) and (2) above are hard disqualifiers. Also try to fill in any gaps in years_in_business, client_count_est, review_rating/review_count, accepting_new_clients, or social_handles if phase 1 left them blank — in particular, if signals.social_handles.linkedin is still missing for this contact, run a search formatted as "[first name] [last name] CPA" specifically to find their linkedin.com/in/... profile URL; you still can't fetch the page, but capture the exact URL from the search result and save it the same way (signals.social_handles = {"linkedin": "<url>"}) for manual review. You get AT MOST 6 web_search calls AND AT MOST 6 web_fetch calls total for this contact — count them as you go. These caps are now enforced mechanically, not just by this instruction: the moment you go over either one, that tool returns a budget-exceeded message instead of actually performing the call, so don't wait to self-regulate — stop as soon as you hit 6 of either and move to the verdict/save step with whatever you have, even if something's still unconfirmed. You have more room here than phase 1 because you're going deeper on one already-verified entity instead of finding one from scratch, so use the budget to actually cover the distinct things listed above (website, in-house-bookkeeping check, no-referral check, trade-client evidence, reviews, gap-filling) rather than repeating variations of the same query. Each call should be going after a genuinely different piece of information. Watch for a specific trap: a firm's name or the person's first name can collide with something totally unrelated (a large company with a similar name, a common first name pulling generic web results, a different business entirely) — if you notice your results are actually about that OTHER thing rather than the specific firm/person you're researching, that is your cue to stop searching and decide with what you have, not to keep rephrasing the query hoping the next one lands — plausible-looking results about the wrong entity are not progress. If you hit a budget cap and genuinely believe more research would change the verdict, don't guess: call crm_enrich_contact with signals.needs_more_research=true and status left at 'researched' (not a final verdict) instead of forcing potential/not_viable on incomplete information — a future phase-2 run will pick this contact back up first, with its own full budget, so nothing you've already found is wasted. Otherwise, call crm_enrich_contact for ONLY your chosen contact_id: set status to 'potential' if no hard disqualifier was found and there's a real positive signal (especially trade-client evidence or explicit referral-partnership language), or 'not_viable' if a hard disqualifier was found or there's simply no positive signal to justify pursuing them. When the soft signals above (remote-friendly, QuickBooks Online, strong communication) stack on top of a real positive signal, say so explicitly in note — the verdict field itself stays binary, but a human will use that detail to prioritize among 'potential' candidates. Save offers_bookkeeping_inhouse, explicit_no_referral, and serves_trade_clients as true/false in signals, plus explicit_referral_partner (true/false) if the firm explicitly states it partners with or refers to other professionals, remote_friendly (true/false, omit if genuinely unclear) for the remote-vs-in-person signal, accounting_software (the specific platform, if one is stated) if mentioned, and communication_quality (a short freeform note, e.g. "responsive, live chat available" or "no clear communication signals found") if you found anything relevant to it — plus whatever else you found. Put your reasoning for the verdict in note — write it so a human can understand the call without re-deriving it. YOU MUST ACTUALLY CALL crm_enrich_contact BEFORE REPLYING — a verdict written only in your reply to me is not saved anywhere in the CRM and will be lost; do not consider this task finished until that tool call has been made. Then send me a short summary: which contact you picked and why, your verdict (or that you flagged it for more research), and one line on why you passed on each of the other 4 based on what was already known about them.
```

### v6 (2026-08-10 – 2026-08-16, superseded by v7 above)

Same LinkedIn-URL-capture fix as phase 1's v6 (see above for full
rationale), added to the phase-2 gap-filling sentence: if
`signals.social_handles.linkedin` is still missing for the chosen contact,
run a `"[first name] [last name] CPA"` search specifically to find it and
save the URL (never fetch it). Everything else unchanged from v5. Not yet
validated against a live batch as of this writing.
...
### v5 (2026-08-06 – 2026-08-10, superseded by v6 above)
